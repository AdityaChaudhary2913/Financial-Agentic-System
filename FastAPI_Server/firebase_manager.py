...
class FirebaseManager:
    ...
    async def save_financial_state(self, user_id, session_id):
        if not self.db:
            logging.error("Realtime Database client not available.")
            return
        try:
            session = await self.session_service.get_session(
                app_name="artha", user_id=user_id, session_id=session_id
            )
            ...
            goals_state_ref = (
                self.db.child("users").child(user_id).child(current_financial_goals)
            )
            goals_state_ref.set(current_financial_goals)

            agent_persona = session.state["agent_persona"]
            persona_state_ref = (
                self.db.child("users").child(user_id).child(agent_persona)
            )
